...
struct timer_list *g_ptimer = NULL;
...
#endif
#ifdef CONFIG_TOUCHSCREEN_USB_XDTOUCH
	{USB_DEVICE(0x04b4, 0x120f), .driver_info = DEVTYPE_XDTOUCH},
#endif
...
#ifdef CONFIG_TOUCHSCREEN_USB_XDTOUCH
static int xdtouch_read_data(struct usbtouch_usb *dev, unsigned char *pkt)
{
#if 0
	dev->x = ((pkt[2] & 0xFF) << 8) | pkt[1];
	dev->y = ((pkt[4] & 0xFF) << 8) | pkt[3];
	dev->touch = pkt[0] & 0x01;
#else
	dev->x = (((pkt[2] & 0xFF) << 8) | pkt[1])/17;
	dev->y = (((pkt[4] & 0xFF) << 8) | pkt[3])/30;
	dev->touch = pkt[0] & 0x01;
#endif
...
#endif
int xdtouch_remove_timer(struct timer_list **ptimer)
{
	struct timer_list *plist;
	plist = g_ptimer;
	if(plist == NULL)
	{
		printk("warning: timer not present!!\n");
		return 1;
	}
	else
	{
...
static void xdtouch_release_btn(unsigned long param)
{
	struct usbtouch_usb *usbtouch = (struct usbtouch_usb *) param;
	
	input_report_abs(usbtouch->input, ABS_MT_TOUCH_MAJOR, usbtouch->touch);
	if (swap_xy) {
	...
	input_report_key(usbtouch->input, BTN_TOUCH, 0); //修改添加部分by刘芳
	input_mt_sync(usbtouch->input);		
	input_sync(usbtouch->input);
	xdtouch_remove_timer(&g_ptimer);
}

int xdtouch_add_timer(struct usbtouch_usb *usbtouch, struct timer_list **ptimer,unsigned long delay)
{
	struct timer_list *plist;
	plist = g_ptimer;
	if(plist == NULL)
	{
		plist = (struct timer_list*)kmalloc(sizeof(struct timer_list),GFP_KERNEL );
		if(plist == NULL)
		{
			printk("error : can't mkmalloc timer_list!!\n");
			return 1;
		}
		else
		{
			init_timer(plist);
			plist->data = (unsigned long)usbtouch;
			plist->expires = jiffies + delay;
			plist->function = xdtouch_release_btn;
			add_timer(plist);
		}
	}
	else
	{
		mod_timer(plist,jiffies+delay);
	}	
	g_ptimer = plist;
	return 0;
}

static void xdtouch_process_pkt(struct usbtouch_usb *usbtouch,
                                 unsigned char *pkt, int len)
{
	...
	input_report_abs(usbtouch->input, ABS_MT_TOUCH_MAJOR, usbtouch->touch);
//      	input_report_key(usbtouch->input, ABS_MT_TRACKING_ID, xdtouch_point->track_id);
//	swap_xy = 1;
	if (swap_xy) {
			input_report_abs(usbtouch->input, ABS_MT_POSITION_X, usbtouch->y);
			input_report_abs(usbtouch->input, ABS_MT_POSITION_Y, usbtouch->x);
	...
	if(usbtouch->touch == 0)
	{
		...
		xdtouch_add_timer(usbtouch,&g_ptimer,HZ/20);
}
...
static int tc45usb_read_data(struct usbtouch_usb *dev, unsigned char *pkt)
{
	...
	dev->touch = pkt[0] & 0x01;
#else
	dev->x = (((pkt[2] & 0xFF) << 8) | pkt[1])/16;
	dev->y = (((pkt[4] & 0xFF) << 8) | pkt[3])/16;
	dev->touch = pkt[0] & 0x01;
...
static struct usbtouch_device_info usbtouch_dev_info[] = {
	...
	[DEVTYPE_XDTOUCH] = {
			.min_xc         = 0x0,
			.max_xc         = 1920,//0xf87f,//0xfff,
			.min_yc         = 0x0,
			.max_yc         = 1080,//0xe87f,//0xfff,
			.rept_size      = 5,
			.read_data		= xdtouch_read_data,
			.process_pkt	= xdtouch_process_pkt,
